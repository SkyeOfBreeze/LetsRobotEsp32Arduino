...
strand_t pStrand = {.rmtChannel = 0, .gpioNum = LED_DRIVER_PIN, .ledType = LED_WS2812B_V3, .brightLimit = 32, .numPixels = LED_DRIVER_COUNT,
...
void SetupRGB(){
  ...
  if (digitalLeds_initStrands(&pStrand, 1)) {
    Serial.println("Init FAILURE LED");
    return;
  }
...
void refreshLeds(){
    for (int i2 = 0; i2 < LED_DRIVER_COUNT; ++i2)
    {
        ...
        pStrand.pixels[i2] = pixelFromRGB(red, green, blue);
    }
